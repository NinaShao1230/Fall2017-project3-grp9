...
# svm_tune <- tune(svm, train_data, train_label, 
#                  kernel="radial", ranges=list(cost=10^(-1:2), gamma=c(.5,1,2)))
# p<-cv_svmTune$best.parameters

# cv_svmTune<-tune.svm(train_data,y=train_label, sampling = "fix",
#          gamma = 2^c(-8,-4,0,4), cost = 2^c(-8,-4,-2,0))
# p<-cv_svmTune$best.parameters
# p
# best.svm <- svm(train_data,train_label,cost = p[,2], gamma = p[,1])
# pred_test <- predict(best.svm,test_data)
# pred_test<- ifelse(pred_test > 0.5,1,0)
# sum(pred_test != test_label)/nrow(test_data)

trainSVM <- function(data, label){
  t=proc.time()
  pca <- prcomp(data,scale = FALSE)
  pca_x = pca$x[,1:100]
  cv_svmTune<-tune(svm, pca_x, train_label, 
                   kernel="radial", ranges=list(cost=10^(-1:2), gamma=c(.5,1,2)),probability = TRUE)
  p<-cv_svmTune$best.parameters
  best.svm <- svm(pca_x,label,kernel="radial",cost = p[,2], gamma = p[,1],probability = TRUE)
